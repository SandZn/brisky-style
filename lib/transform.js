...
exports.properties = {
  transform: {
    ...
    render: {
      static (target, pnode) {
        pnode.style[transform] = target.storeStatic(!target.$ && target.compute(), pnode)
      },
      state (target, state, type, stamp, subs, tree, uid, pid) {
        ...
        pnode.style[transform] = target.storeState(val, state, type, stamp, subs, tree, pid + 'transform', pid)
      }
    },
    properties: {
      x: {
